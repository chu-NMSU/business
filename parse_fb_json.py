...
with codecs.open('fb_data/fb-2011.txt', 'w', 'utf-8') as outfile1, codecs.open('fb_data/fb-2013.txt', 'w', 'utf-8') as outfile2:
    for user in users:
        user_text2013 = ''
        user_text2011 = ''

        with open('fb_data/'+user+'.json') as infile, codecs.open('fb_data/'+user+'-2011.txt', 'w', 'utf-8') as user_outfile1, codecs.open('fb_data/'+user+'-2013.txt', 'w', 'utf-8') as user_outfile2:
            ...
            for i in range(0, len(timeline_arr)):
                ...
                for j in range(0, len(message_arr)):
                    ...
                    if t>=time2012_end and t<=time2013:
                        ...
                        # outfile.write(user+'\t'+time.strftime('%Y\t%m\t%d', t)+'\t'+time.strftime('%Y-%m-%d %H:%M:%S', t)+'\t'+message+'\n')
                        text2013 += message+'\n'
                        user_text2013 += message+'\n'
            user_outfile1.write(user_text2011)
            user_outfile2.write(user_text2013)
